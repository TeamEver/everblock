...
<div id="block-{$block.id_prettyblocks}" class="{if $block.settings.default.force_full_width}container-fluid px-0 mx-0{elseif $block.settings.default.container}container{/if}{$prettyblock_visibility_class}"{if isset($block.settings.default.bg_color) && $block.settings.default.bg_color} style="background-color:{$block.settings.default.bg_color|escape:'htmlall':'UTF-8'};"{/if}>
  ...
  {if isset($block.states) && $block.states}
    {foreach from=$block.states item=state key=key}
      {include file='module:everblock/views/templates/hook/prettyblocks/_partials/spacing_style.tpl' spacing=$state assign='prettyblock_cta_state_spacing_style'}
      <div id="block-{$block.id_prettyblocks}-{$key}" class="block-cta col-12{if $state.css_class} {$state.css_class|escape:'htmlall'}{/if} {if isset($state.background_image.url) && $state.background_image.url} cta_background{/if}" style="{$prettyblock_cta_state_spacing_style}
