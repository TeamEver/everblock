...
{assign var=closedLabel value=$block.settings.closed_label|default:'?'}
...
{capture assign=noBoxesStatus}{l s='No boxes available.' mod='everblock'}{/capture}
{assign var=currentLangId value=$context.language.id}
{assign var=mysteryConfig value=[
    'boxes' => $mysteryBoxes,
    'playUrl' => $link->getModuleLink('everblock','mysteryboxes'),
    'token' => $static_token,
    'startDate' => $startDate,
    'endDate' => $endDate,
...
{assign var=encodedConfig value=$mysteryConfig|json_encode|base64_encode|escape:'htmlall':'UTF-8'}
...
{/if}
<div id="block-{$block.id_prettyblocks}" class="{$containerClass}">
  {if $block.settings.default.force_full_width}
...
<div class="{if $block.settings.default.container}container{/if}" style="{if isset($block.settings.padding_left) && $block.settings.padding_left}padding-left:{$block.settings.padding_left|escape:'htmlall':'UTF-8'};{/if}{if isset($block.settings.padding_right) && $block.settings.padding_right}padding-right:{$block.settings.padding_right|escape:'htmlall':'UTF-8'};{/if}{if isset($block.settings.padding_top) && $block.settings.padding_top}padding-top:{$block.settings.padding_top|escape:'htmlall':'UTF-8'};{/if}{if isset($block.settings.padding_bottom) && $block.settings.padding_bottom}padding-bottom:{$block.settings.padding_bottom|escape:'htmlall':'UTF-8'};{/if}{if isset($block.settings.margin_left) && $block.settings.margin_left}margin-left:{$block.settings.margin_left|escape:'htmlall':'UTF-8'};{/if}{if isset($block.settings.margin_right) && $block.settings.margin_right}margin-right:{$block.settings.margin_right|escape:'htmlall':'UTF-8'};{/if}{if isset($block.settings.margin_top) && $block.settings.margin_top}margin-top:{$block.settings.margin_top|escape:'htmlall':'UTF-8'};{/if}{if isset($block.settings.margin_bottom) && $block.settings.margin_bottom}margin-bottom:{$block.settings.margin_bottom|escape:'htmlall':'UTF-8'};{/if}">
    {if $block.settings.default.container}
        <div class="row">
    {/if}
        <div class="ever-mystery-boxes text-center" data-block-id="{$block.id_prettyblocks}" data-empty-status="{$noBoxesStatus|escape:'htmlall':'UTF-8'}" data-config="{$encodedConfig}">
            {if $block.settings.title}<h3>{$block.settings.title|escape:'htmlall':'UTF-8'}</h3>{/if}
            <div class="ever-mystery-status-message" style="display:none;">
                <div class="ever-mystery-status-text"></div>
            </div>
            <div class="ever-mystery-content">
                {if $customer.is_logged || $isEmployee}
                    {if $block.settings.instructions}<div class="ever-mystery-instructions mb-3">{$block.settings.instructions nofilter}</div>{/if}
                    {if $mysteryBoxes|@count > 0}
                            ...
                            {foreach from=$mysteryBoxes item=box name=mystery}
                                <button type="button" class="ever-mystery-box" data-box-index="{$smarty.foreach.mystery.iteration-1}" aria-label="{l s='Mystery box %s' sprintf=[$smarty.foreach.mystery.iteration] mod='everblock'}">
                                    ...
                                    <span class="ever-mystery-box-back">
                                        ...
                                        <span class="ever-mystery-box-result-message"></span>
                                    </span>
                                </button>
                            {/foreach}
                        </div>
                        <div class="ever-mystery-reveal-message" style="display:none;"></div>
                    {else}
                        <p class="ever-mystery-empty">{l s='No boxes are configured.' mod='everblock'}</p>
                    {/if}
                {else}
                    {if $block.settings.instructions}<div class="ever-mystery-instructions mb-3">{$block.settings.instructions nofilter}</div>{/if}
                    {if $mysteryBoxes|@count > 0}
                    ...
                    {else}
                        <p class="ever-mystery-empty">{l s='No boxes are configured.' mod='everblock'}</p>
                    {/if}
                    <div class="ever-mystery-login mt-3">
                        <button class="btn btn-primary ever-mystery-login-btn" data-target="#everMysteryLoginModal-{$block.id_prettyblocks}">{l s='Log in to play' mod='everblock'}</button>
                    </div>
                    <div class="modal fade ever-mystery-login-modal" id="everMysteryLoginModal-{$block.id_prettyblocks}" tabindex="-1" aria-hidden="true">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <p class="ever-mystery-login-message mb-3">{l s='Log in to open a mystery box!' mod='everblock'}</p>
                                    <form action="{$link->getPageLink('authentication', true)|escape:'htmlall':'UTF-8'}?back={$urls.current_url|escape:'htmlall':'UTF-8'}" method="post" class="ever-mystery-login-form">
                                        <div class="form-group">
                                            <label for="ever-mystery-login-email-{$block.id_prettyblocks}">{l s='Email' mod='everblock'}</label>
                                            <input id="ever-mystery-login-email-{$block.id_prettyblocks}" class="form-control" type="email" name="email" required>
                                        </div>
                                        <div class="form-group">
                                            <label for="ever-mystery-login-password-{$block.id_prettyblocks}">{l s='Password' mod='everblock'}</label>
                                            <input id="ever-mystery-login-password-{$block.id_prettyblocks}" class="form-control js-visible-password" type="password" name="password" required>
                                        </div>
